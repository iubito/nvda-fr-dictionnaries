...
NVDA speech and symbols dictionnaries, focused on French language, using IBMTTS speech synthetizer.
...
* speechdicts: you may find interesting regexp in files starting by a number:
  * `!` and `_`,
  * spreadsheet cells,
  * Non formatted IBAN spoken by group of 4 characters,
  * CamelBumps,
  * diacritics,
...
* Programmation :
  * Je suis développeur de scripts et applications, dans différents langages. La programmation utilise en grande majorité des termes anglais. J'ai donc ajouté ces termes dans le dictionnaire. Par exemple : open, select, update, enable, delete.
  ...
  * Les zéros initiaux répétés sont simplifiés pour  la lecture de nombre binaires : `0x000F`.
  * Support partiel : le tiret de soustraction sera prononcé moins dans des cas simples : `5-x`, `variable - 3`.
  * Prononciation des noms de fonctions et constantes en langages dérivés du C, des noms de balises HTML, des propriétés CSS. Exemple : `sprintf()`, `ksort()`, `memcpy()`, `<title>`, `<textarea>`, `background`, `1.5em`...
...
* Symboles mathématiques, traitement spécial pour le point d'exclamation après un chiffre qui représente la factorielle, traitement spécial du tiret pour les soustractions simples: `5 - 3`.
* Français :
  ...
  * Des voyelles mangé au milieu de mots : e dans épeler, u dans tuer, i dans skier...
...
* Langues étrangères :
  ...
  * Alphabets grec et cyrillique : ici, aucune tentative de prononciation, mais j'ai tenu à mettre ces caractères pour épeler un mot en se déplaçant caractère par caractère, ou pour entendre une anomalie si un pirate remplace un caractère latin par un caractère cyrillique ressemblant. Cela permet de détecter un spam lorsque cette technique est utilisée. NVDA dira "a cyrillique", "d cyrilliquemajuscule ", etc.
    * Je verrai plus tard si je peux faire prononcer une approximation du grec moderne
...
* Noms propres :
  ...
  * noms des pays en anglais : Poland, Romania, India.
...
* Termes en rapport avec la finance :
  * types de comptes bancaires français : PEA, PER...
  * noms de brokers, paires Forex, crypto-monnaies,
  * Les IBAN non formatés sont prononcés par groupe de 4 caractères : `FR7612345123450000012P34512`.
* Quelques acronymes administratifs français, et découpage clair des n° Sécurité Sociale et SIRET.
* Noms de fabricants, de logiciels, de réseaux sociaux, applications en ligne : BitDefender, Facebook, Firefox, GitHub, Samsung...
...
Ce dictionnaire m'est utile au quotidien, et le sera pour mon fils qui va être équipé d'un ordinateur au collège, qui va donc apprendre du français, des mathématiques, de la chimie, de la musique... et utiliser Libre Office pour écrire des formules mathématiques.


## Optimisé pour le synthétiseur IBMTTS
Disclaimer : IBMTTS est un add-on non officiel, certains diront illégal, qui remplace l'utilisation de la synthèse Eloquence qui n'est plus maintenu par Code Factory. J'ai acheté la licence Eloquence, mais face aux bugs courants comme la perte de mémoire de mon n° de licence, j'ai été contraint de passer du côté obscur de la force.

Le synthétiseur IBMTTS est selon moi le plus efficace en terme de temps de réponse, et qui n'apporte pas de reconstitution des acronymes en noms à rallonge comme le font les voix de synthèse de Microsoft.
...
|[02 tableur et IBAN.dic](https://github.com/iubito/nvda-fr-dictionnaries/blob/main/speechdicts/02%20tableur%20et%20IBAN.dic)|1 ou 2 caractères alpha majuscule, suivi d'un nombre, c'est le nom d'une cellule dans Excel / LibreOffice Calc / Google Sheets.<br>Les caractères alpha sont décomposés.<br>Exemple : `AU1` prononcé "= a u 1" et non "au 1".<br /><br />Les IBAN non formatés sont prononcés en découpant par groupe de 4 caractères.|
